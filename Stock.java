...
public class Stock
{
	 ...
	 * volume to zero. Initializes a priority queue for sell orders to an empty PriorityQueue
	 * with a PriceComparator configured for comparing orders in ascending order; initializes
	 * a priority queue for buy orders to an empty PriorityQueue with a PriceComparator
	 ...
	 * prices; the lowest price in a sell order (or "market") and the number of shares
	 * in it (or "none" if there are no sell orders); the highest price in a buy order
	 * (or "market") and the number of shares in it (or "none" if there are no buy orders).
	 * For example:
	 * Giggle.com (GGGL)
	 ...
	 * a message to that trader. For example:
	 *
